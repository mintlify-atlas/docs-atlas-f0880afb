...
2. **Environment-specific `.env` file** - `.env.dev`, `.env.prod`, etc.
   ```bash
   # In .env.prod
   API_TOKEN=prod_token
   ```

3. **Shared `.env` file** - Fallback for shared credentials
   ```bash
   # In .env
   API_TOKEN=shared_token
   ```

4. **Request defaultValues** - Request-level defaults
   ```json
   {
     "defaultValues": {
...
5. **Environment variables in config.json** - Lowest priority
   ```json
   {
     "environments": {
       "dev": {
...
<AccordionGroup>
  ...
  <Accordion title="Wrong credentials used">
    ...
    2. Verify variable resolution priority (runtime > .env.{`<env>`} > .env > defaultValues > config.json)
