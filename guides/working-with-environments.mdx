...
2. **Environment file** - Either `.env` (default environment) or `.env.<environment>` (custom environment)
3. **Request `defaultValues`**
4. **Environment variables in `config.json`**

<Note>
Mailman loads only ONE .env file at a time based on the selected environment. It does not merge multiple .env files.
</Note>
...
```bash
# .env (used for default environment)
API_TOKEN=dev_token
limit=20
```

When you run with the default environment:
...
- `API_TOKEN` = `dev_token` (from .env)
- `limit` = `50` (from -v flag, overrides .env which overrides defaultValues which overrides config.json)
