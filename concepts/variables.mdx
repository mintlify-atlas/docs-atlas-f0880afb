...
<Steps>
  <Step title="Request default values (lowest priority)">
    Variables defined in the `defaultValues` field of the request file:
  ...
  <Step title="Environment variables from config.json">
    Variables defined in the environment configuration:
  ...
  <Step title="Environment file variables (.env)">
    Variables loaded from `.env` or `.env.{environment}` files:
    ...
    Applied in `src/commands/run.ts:110-114`
  </Step>
  
  <Step title="Command-line variables (highest priority)">
    Variables passed via the `--var` flag:
